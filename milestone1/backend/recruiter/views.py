...
from .models import RecruiterProfile, Job, Interview, RecruiterSettings
from .forms import (
...
def analytics_dashboard(request):
    ...
    week_start = (
        current_monday - timedelta(weeks=11)
    )
...
# Compatibility view for older URL name
@login_required
@ratelimit(
    key="user",
    rate="10/m",
...
def priority_candidates(request):
    ...
    job_filter_options = list(
        recruiter_jobs.values("id", "title")
    )
...
def view_candidate_detail(request, candidate_id):
    ...
    experience_match_score = request.GET.get(
...
def edit_interview(request, interview_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            updated_interview.status = "Rescheduled"
